...
import { ScrollText, ArrowRight, Users } from "lucide-react";
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: dict.bylaws.metaTitle,
    description: dict.bylaws.metaDescription,
  };
}
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2">
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card p-6 ring-1 ring-foreground/10">
            <div className="min-w-0">
              <h3 className="text-base font-semibold">{t.viewGovernance}</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {t.viewGovernanceLead}
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card p-6 ring-1 ring-foreground/10">
            <div className="min-w-0">
              <h3 className="text-base font-semibold">
                {dict.advisoryBylaws.h1}
              </h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {dict.advisoryBylaws.subtitle}
              </p>
            </div>
            <Button asChild variant="outline" className="h-10 rounded-xl px-4">
              <Link href={lp("/advisory-bylaws")}>
                <Users className="size-4" />
                {dict.advisoryBylaws.badge}
                <ArrowRight className="size-4" />
              </Link>
            </Button>
          </div>
